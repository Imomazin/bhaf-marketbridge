...
export default async function CorporatePortalLayout({
  children,
}: {
  ...
  if (!session?.user) {
    redirect("/auth/sign-in?next=/portal/corporate");
  }

  const access = await resolveUserAccess(session.user);

  if (access.needsVerification) {
    redirect("/auth/verify-email?next=/portal/corporate");
  }

  if (!canAccessRouteForRole("/portal/corporate", access.role)) {
