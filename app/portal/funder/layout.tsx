...
export default async function FunderPortalLayout({
  children,
}: {
  ...
  if (!session?.user) {
    redirect("/auth/sign-in?next=/portal/funder");
  }

  const access = await resolveUserAccess(session.user);

  if (access.needsVerification) {
    redirect("/auth/verify-email?next=/portal/funder");
  }

  if (!canAccessRouteForRole("/portal/funder", access.role)) {
