...
import { getDemoRfpById } from "@/lib/demoCorporate";

export const dynamic = "force-dynamic";

export default async function RfpDetailPage({ params }: { params: { id: string } }) {
  if (!DB_ENABLED || !prisma) {
    const rfp = await getDemoRfpById(params.id);
    if (!rfp) notFound();
      ...
      <section className="bg-cream-50 py-12 md:py-16">
        <div className="container-edge max-w-3xl">
          ...
          <p className="mt-1 text-sm text-charcoal-500">
            {rfp.ownerOrgName} · {rfp.category}
            {rfp.region && <> · {rfp.region}</>}
            {rfp.budgetUsd && <> · {rfp.budgetUsd}</>}
            {rfp.deadline && <> · closes {rfp.deadline.slice(0, 10)}</>}
          </p>
          ...
          <div className="mt-8 rounded-md border border-cream-200 bg-white p-4 text-sm text-charcoal-600">
            This demo RFP is published and visible on the marketplace board. Supplier response tracking is not shown on
            this demo route yet.
          </div>
        </div>
      </section>
